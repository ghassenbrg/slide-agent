...
```bash
# 1. Install once — no clone, no sudo, registers every supported AI agent
npx --yes --package @slide-agent/core@latest -- slide-agent install

# 2. Create a deck from a prompt
slide-agent create --prompt brief.md --output my-deck.pptx

# 3. Or let your AI assistant drive: ask Copilot, Claude Code, Codex, or
#    Gemini CLI for a PowerPoint — the installed slide-agent skill handles it
```

The generated `.pptx` is fully editable in PowerPoint, validated against the official ECMA-376 schemas, and accompanied by a machine-readable manifest, scene blueprint, and validation report under `artifacts/`.
...
- **`slide-agent: command not found` after installation** — open a new terminal, or add `~/.local/bin` to `PATH`. The VS Code extension calls the launcher by absolute path and does not need `PATH` changes.
- **npm engine errors during install** — check `node --version`; Slide Agent needs Node.js 22.12 or newer. VS Code being current does not update the system Node.js.
- **PowerPoint asks to repair a file** — run `slide-agent validate --input deck.pptx`; the report's `schema-violation` entries name the offending XML part and line. Decks generated by current Slide Agent versions validate cleanly against the official ECMA-376 schemas; please file an issue with the report attached if you see this.
- **`RENDER_DEPENDENCY_MISSING`** — PDF/PNG previews need LibreOffice and Poppler. Install them with `slide-agent install --with-render-deps` or your system package manager; creation, editing, and validation work without them.
- **A chat agent does not see the skill** — restart the chat or the host application after installation; `slide-agent doctor` verifies the registrations.
- **Overlap errors when validating a deck you generated earlier** — keep the `artifacts/` directory next to the deck; the generation manifest stored there carries intentional-overlap metadata, and validation trusts it only while its recorded SHA-256 still matches the file.

## Contributing

See [CONTRIBUTING.md](CONTRIBUTING.md) for the development workflow, architecture overview, testing requirements, and pull-request expectations.
