...
The extension automatically installs the matching shared engine and registers the Slide Agent skills the first time each extension version starts. Node.js 22.12 or newer and npm/npx must be available on PATH. Use **Slide Agent: Install or Update** to retry explicitly, or disable automatic setup with `slideAgent.autoInstall`. Start a new chat or reload VS Code if a chat that was already open does not show the new skill.
